...
function displayBooks(books) {
    ...
    books.forEach(book => {
        ...
        bookCard.innerHTML = `
            ...
            <h3>${title}</h3>
            <p>${author}</p>
            <button class="like-btn" onclick="addToFavorites('${title}', '${author}', '${thumbnail}')">Like</button>
            <button onclick="toggleAuthorInfo(this)">Author Info</button>
...
// Toggle author info display
function toggleAuthorInfo(button) {
...
function addToFavorites(title, author, thumbnail) {
    ...
    favorites.push({ title, author, thumbnail });
